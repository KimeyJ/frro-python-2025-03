...
def suma_cubo_pares_sum_list(numeros: Iterable[int]) -> int:
    ...
    """
    cubos_pares = [numero**3 for numero in numeros if numero**3 % 2 == 0]
...
numeros_al_cubo = list(map(lambda numero: numero ** 3, numeros))


# Escribir una función lambda que permita filtrar todos los elementos pares

numeros_al_cubo_pares = list(filter(lambda numero: numero % 2 == 0, numeros_al_cubo))
...
numeros_ordenada = sorted(numeros, key = lambda numero: numero % 2 == 0) 
